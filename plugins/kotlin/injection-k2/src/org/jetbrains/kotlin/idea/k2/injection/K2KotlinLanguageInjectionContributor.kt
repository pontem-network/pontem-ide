...
class K2KotlinLanguageInjectionContributor : LanguageInjectionContributor {
    ...
    protected open val kotlinSupport: K2KotlinLanguageInjectionSupport? by lazy {
    ...
    private fun unwrapTrims(ktHost: KtElement): KtElement {
        ...
        val callFqn = analyze(callExpression) {
            val symbol = callExpression.resolveCall()?.singleFunctionCallOrNull()?.partiallyAppliedSymbol?.symbol
    ...
    private fun injectWithMutation(host: KtElement): InjectionInfo? {
        ...
        val resolvedTo = property.mainReference?.resolve()
    ...
    private fun injectionForKotlinCall(argument: KtValueArgument, ktFunction: KtFunction, reference: PsiReference): InjectionInfo? {
        ...
        val element = reference.element as? KtElement ?: return null
        return allowAnalysisOnEdt {
            analyze(element) {
                val functionSymbol = element.resolveCall()?.singleFunctionCallOrNull()?.partiallyAppliedSymbol?.symbol
                    ?: element.mainReference?.resolveToSymbol() as? KtFunctionLikeSymbol
                    ?: return null
                ...
                val annotationForParameter = parameterSymbol.generatedPrimaryConstructorProperty?.findAnnotation<LanguageAnnotation>()
                    ?: parameterSymbol.findAnnotation<LanguageAnnotation>()
                    ?: return null
...
@OptIn(KtAllowAnalysisOnEdt::class)
internal fun KtAnnotated.findAnnotation(annotationFqName: FqName): KtAnnotationEntry? {
    if (annotationEntries.isEmpty()) return null

    return allowAnalysisOnEdt {
        analyze(this) {
            annotationEntries.firstOrNull { entry ->
                val annotationConstructorSymbol = entry.calleeExpression?.resolveCall()
                    ?.singleFunctionCallOrNull()?.partiallyAppliedSymbol?.symbol as? KtConstructorSymbol ?: return null
                annotationConstructorSymbol.containingClassIdIfNonLocal?.asSingleFqName() == annotationFqName
            }
        }
    }
}
...
private fun KtAnnotationApplicationWithArgumentsInfo.getStringValueOfArgument(argumentName: String): String? {
    val argumentValueExpression =
        arguments.firstOrNull { it.name.asString() == argumentName }?.expression?.sourcePsi as? KtExpression
            ?: return null
    val argumentAsConstant = argumentValueExpression.evaluate(KtConstantEvaluationMode.CONSTANT_EXPRESSION_EVALUATION)
        .takeUnless { it?.constantValueKind == ConstantValueKind.Error } ?: return null
    return when (argumentAsConstant.constantValueKind) {
        ConstantValueKind.String -> (argumentAsConstant as KtConstantValue.KtStringConstantValue).value
